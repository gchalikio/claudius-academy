...
3. **Update the `<source>` tag** in `index.html`:
   ...
   <video id="video-player" controls preload="metadata">
     <source
       src="presentations/<deck>/assets/videos/<name>.mp4"
       type="video/mp4"
     />
   </video>
   ```
...
7. **Important — screen-share check.**
   When you share the _entire screen_ during a Zoom/Meet/Teams call,
   browser audio does NOT route to the audience by default. To fix:
   - **Zoom:** check "Share computer sound" in the share dialog.
   - **Meet:** share a _Chrome tab_ (which transmits audio) instead of
...
- Don't open the video modal _during_ the intro animation. The intro
